...
fn is_ignore_error_directive(code: &str, dollar_idx: usize) -> Option<usize> {
    ...
    if code[dollar_idx..].starts_with(directive) {
...
impl<'a> ForgeScriptParser<'a> {
    ...
    pub fn new(manager: Arc<MetadataManager>, code: &'a str) -> Self {
        crate::utils::forge_log(
            ...
            "Creating new ForgeScriptParser instance",
        );
        Self {
            manager,
            code,
    ...
    pub fn parse(&self) -> ParseResult {
        ...
        if !offsets.is_empty() {
            let mut result =
                ForgeScriptParser::new_internal(self.manager.clone(), &code_to_parse).parse();
    ...
    fn parse_internal(&self) -> ParseResult {
        ...
        while let Some((idx, c)) = iter.next() {
            ...
            if c == '$' && !is_escaped(self.code, idx) {
                ...
                let (matched, used_end) =
                    self.find_matched_function(&mut iter, &full_name, name_start, name_end);
    ...
    fn handle_backslash_escape(
        ...
        let Some(&(_, next_c)) = iter.peek() else {
            return false;
        };
    ...
    fn handle_js_expression(
        ...
        if let Some(end) = find_matching_brace(self.code, brace_idx) {
            tokens.push(Token {
                ...
                text: self.code[brace_idx + 1..end].to_string(),
                start,
                end: end + 1,
            });
            while let Some(&(j, _)) = iter.peek() {
        ...
        if !ignore {
            diagnostics.push(Diagnostic {
                message: "Unclosed JS expression".into(),
                start,
                end: self.code.len(),
            });
        }
    ...
    fn handle_escape_function(
        ...
        if let Some(&(idx, '[')) = iter.peek() {
            if let Some(close) = find_matching_bracket_raw(self.code.as_bytes(), idx) {
                tokens.push(Token {
                    ...
                    text: self.code[idx + 1..close].to_string(),
                    start,
                    end: close + 1,
                });
                while let Some(&(j, _)) = iter.peek() {
            ...
            if !ignore {
                diags.push(Diagnostic {
                    message: format!("Unclosed '[' for ${name}"),
                    start,
                    end: self.code.len(),
                });
            }
        ...
        if !ignore {
            diags.push(Diagnostic {
                message: format!("${name} requires brackets"),
                start,
                end,
            });
        }
    ...
    fn find_matched_function(
        ...
        if matches!(iter.peek(), Some(&(_, '['))) {
            if let Some(f) = self.manager.get_exact(&key) {
                return (Some((f.name.trim_start_matches('$').into(), f)), end);
            }
        } else if let Some((matched, f)) = self.manager.get_with_match(&key) {
            let m = matched.trim_start_matches('$');
            if full.to_lowercase().starts_with(m) {
                return (
                    Some((f.name.trim_start_matches('$').into(), f)),
                    start + m.len(),
                );
            }
        }
    ...
    fn handle_unknown_function_call(
        ...
        tokens.push(Token {
            kind: TokenKind::Unknown,
            text: self.code[start..name_end].to_string(),
            start,
            end: name_end,
        });
        *last = name_end;
        ...
        if let Some(&(idx, '[')) = iter.peek() {
            if let Some(close) = find_matching_bracket(self.code, idx) {
                ...
                let res =
                    ForgeScriptParser::new_internal(self.manager.clone(), inner).parse_internal();
                for t in res.tokens {
                    tokens.push(Token {
                        kind: t.kind,
                        text: t.text,
                ...
                if !ignore {
                    for mut d in res.diagnostics {
                        d.start += idx + 1;
                        d.end += idx + 1;
                        diags.push(d);
                    }
                    for mut f in res.functions {
                        f.offset_spans(idx + 1);
                        funcs.push(f);
                    }
                }
                tokens.push(Token {
    ...
    fn handle_matched_function(
        ...
        if !has_suffix {
            if let Some(&(idx, '[')) = iter.peek() {
                if let Some(close) = find_matching_bracket(self.code, idx) {
                    args_text = Some(&self.code[idx + 1..close]);
                    args_start = idx + 1;
                    while let Some(&(j, _)) = iter.peek() {
                        if j <= close {
                            iter.next();
        ...
        if let Some(inner) = args_text {
            if meta.brackets.is_some() {
                if let Ok(vec) = parse_nested_args(inner, &self.manager, diags, funcs, args_start) {
                    ...
                    if !ignore && let Some(m_args) = &meta.args {
                        validate_arg_enums(&name, &vec, m_args, &self.manager, diags, self.code);
                    }
                } else if !ignore {
                    diags.push(Diagnostic {
                ...
                diags.push(Diagnostic {
                    message: format!("${name} does not accept brackets"),
                    start,
                    end: *last,
                });
            }
            ...
            diags.push(Diagnostic {
                message: format!("${name} expects brackets"),
                start,
                end: used_end,
            });
        }
        ...
        tokens.push(Token {
            ...
            text: self.code[start..used_end].to_string(),
            start,
            end: used_end,
        });
        if has_suffix {
            tokens.push(Token {
                kind: TokenKind::Text,
                text: self.code[used_end..full_end].to_string(),
                start: used_end,
                end: full_end,
            });
        }
        if !ignore {
            funcs.push(ParsedFunction {
                name: meta.name.trim_start_matches('$').into(),
                matched: self.code[start..used_end].into(),
                args: parsed_args,
                span: (start, if has_suffix { used_end } else { *last }),
                silent,
                negated,
                count: None,
                meta,
...
fn parse_nested_args(
    ...
    while c_idx < chars.len() {
        ...
        if c == '\\'
            && let Some(jump) = try_handle_backslash_escape_in_args(
                &chars,
                c_idx,
                is_start,
                &mut current,
        ...
        match c {
            '[' => {
                if !is_escaped_bracket_in_args(&chars, c_idx)
            ...
            ']' => {
                if depth > 0 && !is_escaped_bracket_in_args(&chars, c_idx) {
                    depth -= 1;
                }
                current.push(c);
                c_idx += 1;
            ...
            ';' if depth == 0 => {
                ...
                if !trimmed.is_empty() {
                    args.push((
                        parse_single_arg(trimmed, manager, first_escaped, diags, funcs, off),
                        (off, off + trimmed.len()),
                    ));
                } else {
                    args.push((
...
fn is_function_call_bracket_sub(input: &str, idx: usize) -> bool {
    if idx == 0 || input.as_bytes().get(idx) != Some(&b'[') {
        return false;
    }
    let mut i = idx;
...
pub(crate) fn is_escaped_bracket_in_args(chars: &[(usize, char)], idx: usize) -> bool {
    ...
    if let (Some(&(_, c1)), Some(&(_, c2))) = (chars.get(idx - 2), chars.get(idx - 1)) {
        c1 == '\\' && c2 == '\\'
    } else {
        false
    }
...
fn try_handle_escape_function_in_args(
    ...
    if input.as_bytes().get(idx) == Some(&b'$') && depth == 0 {
        if let Some(e) = find_escape_function_end(&input[idx..], 0) {
            current.push_str(&input[idx..=idx + e]);
            return Some(e + 1);
        }
    }
    None
...
fn try_handle_backslash_escape_in_args(
    ...
    if let Some(&(_, next)) = chars.get(col + 1) {
        ...
        if next == '\\'
            && let Some(&(_, third)) = chars.get(col + 2)
            && matches!(third, '$' | '[' | ']' | ';' | '\\')
        {
            current.push_str("\\\\");
            current.push(third);
...
fn parse_single_arg(
    ...
    if !force && input.starts_with('$') {
        ...
        for f in &mut res.functions {
            f.offset_spans(base);
            funcs.push(f.clone());
        }
        if let Some(f) = res.functions.first() {
            let mut func = f.clone();
            func.offset_spans(base);
            return smallvec![ParsedArg::Function {
                func: Box::new(func)
            }];
        }
    }
...
fn validate_arg_count(
    ...
    if tot < min {
        diags.push(Diagnostic {
            message: format!("${name} expects >= {min} args, got {tot}"),
            start: span.0,
            end: span.1,
        });
    } else if !rest && tot > max {
        diags.push(Diagnostic {
            message: format!("${name} expects <= {max} args, got {tot}"),
            start: span.0,
            end: span.1,
        });
    }
...
fn validate_arg_enums(
    name: &str,
    parsed: &[(SmallVec<[ParsedArg; 8]>, (usize, usize))],
    meta: &[crate::metadata::Arg],
    mgr: &Arc<MetadataManager>,
    diags: &mut Vec<Diagnostic>,
    _src: &str,
) {
    for (i, (parts, span)) in parsed.iter().enumerate() {
        ...
        let arg = if i < meta.len() {
            &meta[i]
        } else if let Some(last) = meta.last()
            && last.rest
        {
            last
        } else {
        ...
        if let Some(v) = vals
            && parts.iter().all(|p| matches!(p, ParsedArg::Literal { .. }))
        {
            let mut text = String::new();
            for p in parts {
                if let ParsedArg::Literal { text: t } = p {
                    text.push_str(t);
                }
            }
            ...
            if !v.contains(&text) {
                diags.push(Diagnostic {
                    message: format!(
                        "Invalid value `{text}` for `{}`. Expected: {:?}",
                        arg.name, v
                    ),
                    start: span.0,
                    end: span.1,
